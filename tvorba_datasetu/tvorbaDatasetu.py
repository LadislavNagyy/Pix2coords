...
# Dataset parameters
NUM_IMAGES = 18  # pocet vygenerovanycg grafov (idealne musi byt delitelne poctom STYL_BODOV)
...
OUTPUT_DIR_data = "./tvorba_datasetu/data_na_trenovanie/data"
OUTPUT_DIR_figures = "./tvorba_datasetu/data_na_trenovanie/figures"
OUTPUT_DIR_test = "./tvorba_datasetu/data_na_trenovanie/data/test"
OUTPUT_DIR_train = "./tvorba_datasetu/data_na_trenovanie/data/train"
OUTPUT_DIR_valid = "./tvorba_datasetu/data_na_trenovanie/data/valid"
...
def generovanie_nahodnych_nespojitych_bodov():
    ...
    x_bod = np.random.randint(RANGE, IMG_WIDTH, size=num_points).tolist()
    y_bod = np.random.randint(RANGE, IMG_HEIGHT, size=num_points).tolist()
    return x_bod, y_bod
...
def konverzia_suradnic(x_ciara=0, y_ciara=0, x_body=0, y_body=0, velkost_bodu=0, styl_bodu=''): 
    fig, graf = plt.subplots(figsize=(IMG_WIDTH/DPI, IMG_HEIGHT/DPI), dpi=DPI)                          # figsize je v zaklade v placoch,takze je potrebna konverzia na pixel
    graf.plot(x_ciara, y_ciara, linestyle=nahodny_styl_ciary(), color = nahodna_farba(), linewidth = nahodna_hrubka_ciary())
    graf.scatter(x_body, y_body, color = nahodna_farba(), s = velkost_bodu, marker = styl_bodu, zorder=4)  
    ...
    # je to potrebné, keze sa samozná kniznica matplotlib neposkytuje pozadovanú presnoť
    if styl_bodu==".":
    ...
    return centersPix, pointDimensionPix, img_path_figures
